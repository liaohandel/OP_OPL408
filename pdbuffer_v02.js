...
function devalarmbuff(alarmcmd){//fcc10681019f010381
	...
	if(nsdevadd == 0x21){
		...
		if(sdevcmd == "07"){
			...
			update_alarmcodeurl= "http://tscloud.opcom.com/Cloud/API/v2/Alarm"+"?ID="+pdbuffer.setuuid+"&POS="+apos+"&Type="+atype+"&value="+alarmcode+"&Data=0";
			...
			updateipc_alarmcodeurl= "http://192.168.5.220/API/v2/Alarm.php"+"?ID="+pdbuffer.setuuid+"&POS="+apos+"&Type="+atype+"&value="+alarmcode+"&Data=0";
	...
	if((nsdevadd>=0x2f) && (nsdevadd<=0xe0) && (sdevcmd=="07")){//device Fail alarm addposmap
		...
		apos = xpdjobj.PDDATA.addposmap[sdevadd];
		
		//alarm code 0x4001 => by POS device list to upload
		update_alarmcodeurl= "http://tscloud.opcom.com/Cloud/API/v2/Alarm"+"?ID="+pdbuffer.setuuid+"&POS="+apos+"&Type="+apos+"&value=4001&Data=0";
		console.log(">>alarm update to web DB =>"+update_alarmcodeurl);
		client.get(update_alarmcodeurl,cargs, function (data, response) {
			console.log("alarm code active update to webDB   ok ...");
		}).on("error", function(err) {console.log("err for client");}).on('requestTimeout', function (req) {req.abort();});
		...
		updateipc_alarmcodeurl= "http://192.168.5.220/API/v2/Alarm.php"+"?ID="+pdbuffer.setuuid+"&POS="+apos+"&Type="+apos+"&value=4001&Data=0";
		console.log(">>alarm update to web DB =>"+updateipc_alarmcodeurl);
		client.get(updateipc_alarmcodeurl,cargs, function (data, response) {
			console.log("alarm code active update to webDB   ok ...");
		}).on("error", function(err) {console.log("err for client");}).on('requestTimeout', function (req) {req.abort();});
